...
class ExampleQModel(QModel[ExampleData]):
    ...
    def __init__(self, project_dir: Path | None, data: ExampleData | dict[str, object] | None) -> None:
        super().__init__(model_cls=ExampleData, project_dir=project_dir, data=data)
        self._update_path_field("path", None)
    ...
    def path(self) -> Path | None:
        return self._make_path_absolute(self._data.path) if self._data.path else None

    @path.setter
    def path(self, value: Path | None) -> None:
        self._set_path_field("path", value)

    def set_project_dir(self, project_dir: Path | None) -> None:
        old_project_dir = self._project_dir
        super().set_project_dir(project_dir)
        self._update_path_field("path", old_project_dir)
...
class TestQModel:
    ...
    def test_path_attr_stores_relative_path_when_resource_is_inside_project_dir(self, tmp_path: Path) -> None:
        ...
        model = ExampleQModel(project_dir, data)
    ...
    def test_path_attr_stores_absolute_path_when_project_dir_is_none(self, tmp_path: Path) -> None:
        ...
        data = ExampleData(name="test", path=resource)
        model = ExampleQModel(project_dir=None, data=data)
    ...
    def test_set_project_path_updates_stored_path_to_absolute_when_outside_project(self, tmp_path: Path) -> None:
        ...
        data = ExampleData(name="test", path=resource)
        model = ExampleQModel(project1_dir, data)
